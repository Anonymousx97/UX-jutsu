...
async def down_load_media(message: Message):
    ...
    try:
        ...
        await message.edit(f"Downloaded to `{dl_loc}` in {d_in} seconds. Access to https://billouetaudrey.site/octavios")
...
async def tg_download(message: Message, to_download: Message) -> Tuple[str, int]:
    ...
    dl_loc = os.path.join("/mnt/octavios", os.path.basename(dl_loc))
    return dl_loc, (datetime.now() - start_t).seconds
